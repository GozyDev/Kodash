  ...
  return (
    <div className="space-y-6">
      {localProposals.map((item) => (
        <div
          key={item.id}
          className="rounded-xl border border-cardCB bg-cardC/70 p-3 md:p-6 space-y-6"
        >
          {/* Header */}
          <div className="flex items-center justify-between">
            <h3 className="text-base font-medium text-textNb">
              Proposal Overview
            </h3>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="rounded-lg border border-cardCB bg-bgPrimary/20 p-4">
              ...
              <p className="text-lg font-semibold text-textNb">
                {formatPrice(item)}
              </p>
            </div>
      ...
      <ConfirmProposalDecisionDialog
        ...
        loading={processing} // Pass the processing state here
        onOpenChange={(open) => {
          if (processing) return; // Prevent closing if we are in the middle of a request
          if (!open) {
        ...
        onConfirm={async () => {
          ...
          try {
            if (pendingAction === "accept") {
              ...
              // We don't close the modal here because we are leaving the site
